...
function adicionarItem(produto) {
  ...
  if (idx >= 0) {
    ...
    const fator         = parseFloat(produto.fator_conversao || 1);
    const unidCompra    = produto.unidade_compra || 'caixa';
    const unidVenda     = produto.unidade_venda  || 'unidade';
    itens.push({
...
function renderItens() {
  ...
  tbody.innerHTML = itens.map((item, idx) => {
    ...
    const uCompra   = item.unidade_compra  || '';
    const uVenda    = item.unidade_venda   || '';
    const stockEntrada = item.quantidade * fator;
    const infoConversao = fator > 1
      ? `<div style="font-size:10px;color:#16a34a;margin-top:2px"><i class="bi bi-arrow-left-right"></i> ${item.quantidade} ${uCompra} × ${fator} = <strong>${stockEntrada} ${uVenda}</strong> em stock</div>`
      : '';
    return `
    <tr>
      <td class="ps-3">
      ...
      <td class="text-center">
        ...
        ${uCompra ? `<div style="font-size:10px;color:#6b7280">${uCompra}</div>` : ''}
      </td>
      <td class="text-end">
        ...
        ${fator > 1 ? `<div style="font-size:10px;color:#6b7280">${formatMZN(item.preco_unitario/fator)}/${uVenda}</div>` : ''}
      </td>
      <td class="text-center">
